...
describe('codeQLInstaller', () => {
  ...
  describe('findBundleAsset', () => {
    it('should prefer .tar.gz over .tar.zst', () => {
      const installer = CodeQLInstaller as any;
      const mockAssets = [
        { name: 'codeql-bundle-linux64.tar.zst', size: 1000, browser_download_url: 'test-zst-url' },
        { name: 'codeql-bundle-linux64.tar.gz', size: 2000, browser_download_url: 'test-gz-url' },
      ];
    ...
    it('should return null if no matching bundle is found', () => {
      const installer = CodeQLInstaller as any;
      const mockAssets = [
        { name: 'some-other-file.tar.gz', size: 1000, browser_download_url: 'test-url' },
      ];
